    ...
    (go
      (while true
        (let [_ (<! chan-update-tags)]
          (POST "/tc/rb/webdocs/webdoctags-edit-table"
                {:params {:id (@page-state :id)}
                 :format :json
                 :response-format :json
                 :keywords? true
                 :error-handler ix/error-handler
                 :handler (fn [webdoctag-edit-table]
    ...
    (go
      (while true
        (let [_ (<! chan-save-tags)]
          (POST "/tc/rb/webdocs/save"
                {:params {:webdoc-row {:id (@page-state :id)}
           ...
           {:title (str "Удаление записи #" (:id row))
            ...
            :footer (list
                     [:button {:class "btn btn-danger btn-lg"
                               :type "button" :data-dismiss "modal"
                               :on-click (fn []
                                           (POST "/tc/rb/webdocs/files_rel/delete"
                                                 {:params {:webdoc-id (@page-state :id)
