(ns ix.omut.component.images-gallery-1
  (:require-macros [cljs.core.async.macros :refer [go]])
  (:require [cljs.core.async :refer [put! chan <!]]
            [om.core :as om :include-macros true]
            [om.dom :as dom :include-macros true]
            [ix.omut.core :as c]
            [ix.omut.component.image-full-screen-viewer :as image-full-screen-viewer]))
            ...
            (dom/div
             ...
             (dom/div #js {:className "btn-toolbar" :style #js {:float "right"}}
                      (dom/div #js {:className "btn-group"}
                               ...
                               (dom/div #js {:className "btn-group"}
                                        (c/ui-button
                                         {:text     (c/ui-glyphicon "fullscreen")
                                          :type     :default
                                          :size     :lg
                                          :on-click #(put! image-full-screen-viewer/chan-show {:src src})})))

                      (dom/div #js {:className "btn-group"}
                               (c/ui-button {:text     (c/ui-glyphicon "chevron-left")
                                             :type     :default
                                             :size     :lg
                                             :on-click (fn [_]
                                                         (om/update-state!
                                                         ...
                                                         1)})
                               (c/ui-button {:text     (c/ui-glyphicon "chevron-right")
                                             :type     :default
                                             :size     :lg
                                             :on-click (fn [_]
                                                         (om/update-state!
