...
(defn component
  [app own
   {:keys [class+
           search-data-fn]
    :or   {search-data-fn (fn [chan-ret-data text]
                            (println '>>> text)
                            (->> {}
                                 (repeat 10)
                                 (map-indexed
                                  (fn [i row]
                                    (assoc row :text (str text " " i))))
                                 vec
                                 (put! chan-ret-data)))}
    :as   opts}]
  ...
  (reify
    ...
    om/IRenderState
    (render-state [_ {:keys [chan-update chan-ret-data
                             in-progress? show-popup?]}]
      (letfn [(search []
                       ...
                       (let [selected (get-selected)]
                         (if (empty? selected)
                           ...
                           (-> app
                               ...
                               (update-in [:input] input/set-value! (:text selected)))))))
                    (om/set-state! own :show-popup? false))
        ...
        (let [app-v @app]
          (dom/div
           ...
           (let [[icon text-class] (if (= (get-in app-v [:selected :text])
